...
function Login() {
  const handleAudio = () => {
    if (!speechSynthesis.speaking) {
      let utterance = new SpeechSynthesisUtterance("Loading face recognition model");
  ...
  const handleVoiceLoadModel = () => {
    if (!speechSynthesis.speaking) {
      let utterance = new SpeechSynthesisUtterance("The model is successfully loaded. Press enter to continue");
      utterance.lang = 'vn-VN';
      speechSynthesis.speak(utterance);
    }
  }
  ...
  const handleKeyboardPress = (event) => {
    if (event.key === 'Enter') {
      var link = document.getElementById("StartLoginButton");
      console.log(link);
      link.click();
    }
  };
  ...
  return (
    <>
      <div className="h-[calc(100vh)] bg-gradient-to-t from-blue-400 to-blue-50">
        ...
        <div className="content3 h-full flex flex-col items-center justify-center gap-[24px] max-w-[720px] mx-auto">
          {!localUserStream && !modelsLoaded && (
            <h2 className="text-center text-3xl font-extrabold tracking-tight text-gray-900 sm:text-4xl">
              <span className="block">
                Your are logging in using face recognition
              </span>
              <span className="block text-blue-900 mt-2">Loading face recognition model...</span>
            </h2>
          )}
          {!localUserStream && modelsLoaded && (
            <h2 className="text-center text-3xl font-extrabold tracking-tight text-gray-900 sm:text-4xl">
          ...
          {localUserStream && loginResult === "FAILED" && (
            <h2 className="text-center text-3xl font-extrabold tracking-tight text-rose-700 sm:text-4xl">
              <span className="block mt-[56px]">
                We cannot identify your face.
              </span>
            </h2>
          )}
          {localUserStream && !faceApiLoaded && loginResult === "PENDING" && (
            <h2 className="text-center text-3xl font-extrabold tracking-tight text-gray-900 sm:text-4xl">
              <span className="block mt-[56px]">Face regonizing...</span>
            </h2>
          )}
